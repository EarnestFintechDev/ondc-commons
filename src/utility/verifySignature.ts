...
const getProviderEncryptionPublicKey = async (
  providers: any,
  keyId: string,
) => {
  try {
...
export const lookupRegistry = async (
  ...
  try {
    ...
    const response = await FetchRequest(
      process.env.GATEWAY_LOOKUP_URL || "",
      "POST",
      body,
    );
    ...
    if (!public_key) {
      log.debug("No public key found", "lookup registry", {
        domain,
        subscriber_id,
        unique_key_id,
      });
    ...
    log.error("Error in lookup", "lookupRegistry", err, {
      subscriber_id,
      unique_key_id,
      domain,
    });
...
const getEncryptionPublicKey = async (
  ...
  try {
    ...
    log.debug("Response from lookup", "lookupRegistry", response);

    if (!response) return false;

    const public_key = await getProviderEncryptionPublicKey(
      response,
      unique_key_id,
    );
    if (!public_key) {
...
export const verifySignature = async (header: any, body: any) => {
  try {
    ...
    if (algorithm === headerParts.algorithm) {
      const public_key = await lookupRegistry(
        subscriber_id,
        unique_key_id,
        domain,
      );
    ...
    log.error("Error in verify signature", "verifySignature", err, {
      header,
      body,
    });
    return false;
...
export const encryptData = async (
  ...
  try {
    ...
    if (!publicKey) {
      log.warn("Error getting public key", "encryptData", {
        publicKey,
        headerParts,
      });
    ...
    const encryptedString = aes256GcmEncrypt(
    ...
    log.error("Error in encrypting data", "encryptData", e, {
      message: e.message,
      data,
      header,
      domain,
