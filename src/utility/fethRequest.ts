...
export const FetchRequest = async (url: string, method: string, body: any) => {
  ...
  log.debug("Looking for header after calling getHeader in utility/fetchRequest.ts -> ", "getHeader()", { Header })
  const option: object = {
    method: method,
    headers: Header,
    body: body,
  ...
  return await fetch(url, option)
    ...
    .then((data) => log.debug("Response from FetchRequest function", "FetchRequest", data))
    .catch((err) => {
      log.error(
